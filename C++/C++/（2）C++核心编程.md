...
int main() {
	...
	int a = 10;//a是变量，可读可写，const是只读不可写
	func(a); //调用无const的func
	func(10);//调用有const的func，若调用第一个，则为int &a=10，不合法
	//const int &a =10 <==> int temp=10; int &a=temp;

	//func2(10);
    //碰到默认参数产生歧义，需要避免（可以调第一个，也可以调第二个）
